...
class ASRServer:
    ...
    def __init__(self, store: Optional[ASRComponentsStore] = None):
        """
    ...
    def _setup_routes(self) -> None:
        ...
        async def upload_and_transcribe(audio_file: UploadFile = File(...)):
            ...
            async def transcription_generator():
                ...
                try:
                    async for result in self._transcribe_audio_in_chunks(upload_processor, audio_bytes):
                        data = json.loads(result)
                        if "text" in data:
                            current_full_transcript.append(data["text"])
                            payload = {
                                "full_transcript": " ".join(current_full_transcript),
                                "segments": [{"start": data.get("start", 0), "end": data.get("end", 0), "text": data["text"]}],
    ...
    def _create_upload_processor(self, processor_template: ASRProcessor) -> ASRProcessor:
...
def create_app(store: Optional[ASRComponentsStore] = None) -> FastAPI:
    """
