...
from real_time_asr_backend.backends_for_whisper_online import (
    ASRBase,
    MLXWhisper,
...
class OnlineASRProcessor:
    ...
    def _transcribe_audio(self) -> tuple[Any, bool]:
        ...
        try:
            ...
            if isinstance(result, dict) and "segments" in result:
                ...
                logger.warning(
                    f"ASR returned an unexpected structure: {type(result)}. Treating as failure."
                )
    ...
    def _trim_buffer_by_segment(self, asr_result: Any):
        ...
        for seg_end in sorted(segment_ends_relative, reverse=True):
            ...
            if (
                seg_end_abs <= last_committed_time_abs
    ...
    def _apply_fallback_trim(self, asr_success: bool):
        ...
        is_failing = (
            not asr_success
            and self.consecutive_asr_failures >= self.MAX_CONSECUTIVE_ASR_FAILURES
        )
        is_too_long = buffer_len_after_trim > (
            self.buffer_trimming_sec + self.FALLBACK_TRIM_THRESHOLD_SEC
        )
    ...
    def _chunk_at_timestamp(self, time: float):
        ...
        if (
            cut_seconds <= 0
            or cut_seconds > len(self.audio_buffer) / self.SAMPLING_RATE
        ):
            return
        ...
        logger.debug(
            f"Trimming {cut_seconds:.2f}s from audio buffer. New offset: {time:.2f}"
        )
...
def asr_factory(
    args: argparse.Namespace, logfile=sys.stderr
) -> tuple[ASRBase, OnlineASRProcessor]:
    """
    ...
    if not asr_cls:
        ...
        if backend_name in ["openai-api", "faster-whisper"]:
            raise NotImplementedError(
                f"The '{backend_name}' backend is not yet implemented."
            )
        raise ValueError(
    ...
    t = time.time()
    logger.info(
        f"Loading Whisper model '{args.model}' for language '{args.lan}' using '{backend_name}' backend..."
    )
    ...
    if args.vac:
        raise NotImplementedError(
            "Voice Activity Controller (VAC) is not yet implemented."
        )
