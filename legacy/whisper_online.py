...
class OnlineASRProcessor_old:
    ...
    def process_iter(self):
        ...
        # --- drop the audio we just processed, regardless of whether
        # flush(o) returned anything or not ---
        # chunk_at takes an absolute timestamp, so:
        # chunk_time = self.buffer_time_offset + buffered_sec
...
class OnlineASRProcessor:
    ...
    def __init__(
        ...
        self.MAX_CONSECUTIVE_ASR_FAILURES_FOR_FALLBACK_TRIM = (
            3  # Configurable: after how many failures to force trim
        )
    ...
    def prompt(self):
        ...
        non_prompt = self.commited[k:]
        return self.asr.sep.join(prompt_list[::-1]), self.asr.sep.join(
            t for _, _, t in non_prompt
        )
    ...
    def process_iter(self):
        ...
        empty_asr_result = {
            "text": "",
            "segments": [],
            "language": self.asr.original_language or "en",
        }
        ...
        try:
            ...
            if isinstance(temp_res, dict) and "segments" in temp_res:
                ...
                tsw = self.asr.ts_words(
                    res
                )  # This might also fail if res structure is unexpected despite 'segments' key
                asr_success = True
            ...
            logger.error(
                f"AssertionError during asr.transcribe: {e}. Treating as failure."
            )
            # res remains empty_asr_result, tsw remains []
            self.consecutive_asr_failures += 1
        except Exception as e:  # Catch other potential errors from transcribe
            logger.error(
                f"Generic error during asr.transcribe: {e}. Treating as failure."
            )
            # res remains empty_asr_result, tsw remains []
            self.consecutive_asr_failures += 1

        self.transcript_buffer.insert(
            tsw, self.buffer_time_offset
        )  # tsw is [] if ASR failed
        o = (
            self.transcript_buffer.flush()
        )  # o will be [] if ASR failed (no new words inserted)
        ...
        if current_buffer_len_sec > trim_attempt_threshold_sec:
            ...
            if trimmed_by_segment_logic:
                ...
                logger.debug(
                    f"chunk_completed_segment did not trim. Buffer length: {current_buffer_len_sec:.2f}s"
                )
        ...
        fallback_trigger_threshold_sec = (
            self.buffer_trimming_sec + self.FALLBACK_TRIM_OVER_THRESHOLD_SEC
        )

        if (
            not asr_success
            and self.consecutive_asr_failures
            >= self.MAX_CONSECUTIVE_ASR_FAILURES_FOR_FALLBACK_TRIM
            and current_buffer_len_sec_after_trim > fallback_trigger_threshold_sec
        ):
            ...
            samples_to_discard = int(
                self.min_chunk_sec * self.SAMPLING_RATE
            )  # Discard one min_chunk_sec
            ...
            if samples_to_discard > 0 and samples_to_discard <= len(self.audio_buffer):
                ...
                self.commited = [
                    item for item in self.commited if item[1] > self.buffer_time_offset
                ]
            elif samples_to_discard == 0:
                logger.warning(
                    "Fallback trim: min_chunk_sec is zero, no audio discarded by fallback."
                )
        ...
        logger.debug(
            f"End of process_iter. len of buffer now: {len(self.audio_buffer) / self.SAMPLING_RATE:.2f}"
        )
        return self.to_flush(o)

    def chunk_completed_sentence(self):
        if not self.commited:
            return False  # Ensure self.commited is not empty
    ...
    def chunk_completed_segment(
        self, res
    ):  # res can be an actual ASR result or empty_asr_result
        if not self.commited:
            logger.debug(
        ...
        if not (isinstance(res, dict) and "segments" in res):
            ...
            current_asr_segment_ends = self.asr.segments_end_ts(
                res
            )  # e.g., [0.5, 1.2, 2.0] relative to current audio_buffer start
        ...
        if current_asr_segment_ends:
            ...
            for seg_end_relative in sorted(current_asr_segment_ends, reverse=True):
                ...
                if (
                    seg_end_abs <= last_committed_word_end_time_abs
                    and seg_end_abs > self.buffer_time_offset
                ):
    ...
    def chunk_at(self, time):
        ...
        if cut_samples <= 0:
            logger.debug(
                f"chunk_at: Calculated zero or negative samples to cut ({cut_samples}). No cut made."
            )
            return
    ...
    def words_to_sentences(self, words):
        ...
        try:
            ...
            logger.error(
                f"Error during sentence tokenization: {e}. Treating input as a single sentence."
            )
            if not words:
                return []
        ...
        for sent_text in segmented_sentences_text:
            ...
            for i in range(start_idx_for_this_sentence, len(words)):
                ...
                if (
                    not temp_sent_text_matcher
                ):  # Should not happen if sent_text_clean was not empty
                    break
                ...
                if temp_sent_text_matcher.startswith(word_text_clean):
                    ...
                    accumulated_words_for_sentence.append(
                        word_text
                    )  # Use original word text for rejoining
                    sentence_end_time = word_end_time  # Keep updating end time

                    # Remove matched part
                    temp_sent_text_matcher = temp_sent_text_matcher[
                        len(word_text_clean) :
                    ].strip()
                    current_word_idx = (
                        i + 1
                    )  # Advance current_word_idx past this word for next sentence
                    ...
                    if (
                        not accumulated_words_for_sentence
                    ):  # First word of sentence didn't match
            ...
            if sentence_start_time is not None and sentence_end_time is not None:
                ...
                out_sentences.append(
                    (sentence_start_time, sentence_end_time, final_sentence_text)
                )
            elif (
                sent_text_clean
            ):  # Tokenizer gave a sentence, but we couldn't align it with words
                logger.warning(
                    f"Could not align tokenized sentence '{sent_text_clean}' with ASR words."
                )
    ...
    def to_flush(
        self, sents, sep=None, offset=0
    ):  # offset is not used as sents have absolute timestamps
        if sep is None:
            sep = self.asr.sep
        t = sep.join(s[2] for s in sents)
        if not sents:
