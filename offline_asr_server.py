...
def update_table(timeline):
    ...
    table_data = [
        [
            "client" if item.get("group") == 0 else "agent",  # Map group 0 to "client", others to "agent"
            item.get("content"),
            item.get("start"),
            item.get("end"),
...
def build_transcript_from_words(word_timestamps, duration):
    ...
    transcript = {
        ...
        "items": [
            {"content": "", "group": "track-length", "selectable": False, "type": "background", "start": 0, "end": int(duration * 1000), "className": "color-primary-600"},
        ]
    }
...
with gr.Blocks() as demo1:
    ...
    with gr.Row(equal_height=True):
        ...
        with gr.Column(elem_id="lang-select"):
            gr.HTML("<p><b>Step 2:</b> Choose the input and output language.</p>", show_label=False)
    ...
    with gr.Row():
        ...
        with gr.Column():
            gr.HTML("<p><b>Step 3:</b> Upload an audio file or record with your microphone.</p>")
    ...
    with gr.Row():
        spectrogram_plot = gr.Plot(label="Mel Spectrogram", elem_id=SPECTOGRAM_ID, container=True)

    with gr.Row():
        dateless_timeline = VisTimeline(
            value={
                "groups": [{"id": "track-length", "content": ""}, {"id": 1, "content": ""}],
                "items": [
                    {"content": "", "group": "track-length", "selectable": False, "type": "background", "start": 0, "end": 60000, "className": "color-primary-600"},
                ],
            },
    ...
    go_button.click(
        ...
        fn=generate_interactive_melspec,
        inputs=[audio_file],
